...
export default async function GraphicDesign() {
  const embeds = await getEmbeds("graphic-design");
    ...
    <div className="pt-[72px] min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      <div className="px-6 md:px-12 pt-20 pb-14 max-w-7xl mx-auto border-b border-slate-200 dark:border-slate-800">
        ...
        <h2 className="font-sans text-[clamp(40px,6vw,72px)] font-light leading-none text-slate-900 dark:text-slate-50 reveal d1 in transition-colors duration-300">
          Visual<br /><em className="italic text-brand-500">Gallery</em>
        </h2>
      </div>
        ...
        {embeds.length === 0 ? (
          <div className="py-32 flex flex-col items-center justify-center text-center">
            ...
            <h3 className="font-sans text-3xl font-light text-slate-900 dark:text-slate-50 mb-4 transition-colors duration-300">
              <em className="italic text-brand-500">bankiim_design</em> is currently
            </h3>
            ...
            <p className="text-slate-500 dark:text-slate-400 max-w-md transition-colors duration-300">
              I'm curating my best visual pieces. Check back soon for updates.
            </p>
          </div>
        ) : (
          <div className="mt-16 columns-1 md:columns-2 gap-8 space-y-8">
            {embeds.map((embed) => (
              <div key={embed.id} className="break-inside-avoid relative overflow-hidden rounded-[2px] shadow-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-2 transition-colors duration-300" dangerouslySetInnerHTML={{ __html: embed.code }} />
            ))}
          </div>
        )}
      </div>
